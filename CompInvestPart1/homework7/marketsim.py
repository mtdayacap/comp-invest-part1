...
def get_actual_price(dt_start, dt_end, symbols):
    ...
    for symbol in symbols:
        df_data[symbol] = df_data[symbol].fillna(method='ffill')
        df_data[symbol] = df_data[symbol].fillna(method='bfill')
        df_data[symbol] = df_data[symbol].fillna(1.0)
...
def convert_orders_csv_to_dataframe(np_orders):
    ...
    df_orders = pd.DataFrame({'datetime':datetimeindex,'symbol': symbols, 'order': orders, 'share':shares})
...
def sim(f_cash, orders_filepath, values_filename):
    ...
    dt_end = date_list[len(date_list) - 1]  #End Date
    ...
    for dti in datetimindex_daily:
        ...
        if(any(df_orders.datetime == dti)):
            df_dt_orders = df_orders[df_orders.datetime == dti]
            ctr = df_dt_orders.index
            #log_debug(str(df_dt_orders))
            for i in ctr:
                order = df_dt_orders['order'].ix[i]
                symbol = df_dt_orders['symbol'].ix[i]
                shares = int(df_dt_orders['share'].ix[i])
                sym_closing_price = df_closing_prices[symbol].ix[dti]
                ...
                #log_debug("order="+str(order)+",symbol="+str(symbol)+",shares="+str(shares)+",closing_price="+str(sym_closing_price)+",ownership="+str(ownership)+",cash="+str(cash))
